...
keymap(
    ...
    { silent = true, expr = true, desc = "Down" }
)
keymap(
    { "n", "v", "x" },
    "k",
    function() return vim.v.count > 0 and "k" or "gk" end,
    { silent = true, expr = true, desc = "Up" }
)
keymap(
    { "n", "v", "x" },
    "<Down>",
...
keymap(
    ...
    { silent = true, desc = "Current diagnostic" }
)
keymap(
    "n",
    "[d",
...
--
-- CTRL + click on URL.
keymap("n", "<C-LeftMouse>", "<LeftMouse><cmd>normal gx<CR>", { desc = "Click `gx`" })
